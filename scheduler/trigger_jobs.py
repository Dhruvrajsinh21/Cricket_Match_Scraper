...
def trigger_real_time_updates():
    ...
    while True:
        ...
        for match in matches:
            ...
            # Round current time and match start time to the nearest minute (ignoring seconds)
            ...
            # Debug: Print both times to see their values
            print(f"Current UTC time: {current_time_utc}")
            print(f"Match start time (UTC): {start_time_utc}")

            # Add a tolerance window of 5 minutes, for example
        ...
        # Use asyncio.sleep for better async behavior
        asyncio.run(asyncio.sleep(60))  # Replace time.sleep(60) with asyncio sleep
